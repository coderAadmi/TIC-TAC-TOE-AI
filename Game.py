...
class Game():
    def __init__(self):
        ...
        self.font = pg.font.Font(self.font_name, 40)
        self.meta_screen = pg.transform.scale(pg.image.load("black.jpg"), (610, 205))
        self.player_icon = [pg.transform.scale(pg.image.load("yellowCoin.png"), (170, 170)),
                            pg.transform.scale(pg.image.load("redCoin.png"), (170, 170))]
        self.invalid_move_sound = pg.mixer.Sound("shootLaser.wav")
        #pg.mixer.music.load("background.mp3")
        #pg.mixer.music.play(-1)
        self.game_state_screens= [pg.transform.scale(pg.image.load("game_tie.png"),(610,610)),pg.transform.scale(pg.image.load("player_win.png"),(610,610)),pg.transform.scale(pg.image.load("computer_win.png"),(610,610))]
    ...
    def make_move(self):
        for pat in self.knowledge_base:
            if self.matches(pat) and pat not in self.pattern:
                ...
                print('removing : ',pat)
        #print('Patterns ::',self.pattern)
        if self.pattern and self.moves>0:
            min = self.pattern[0]
            ...
            while(self.game_array[rand-1]!=0):
                rand = random.randrange(1, 10)
    ...
    def drawScore(self):
        ...
        self.meta_screen = pg.transform.scale(pg.image.load("black.jpg"), (610, 205))
        score = f'PLAYER: {self.player_score[0]}  COMPUTER :{self.player_score[1]}'
        text = self.font.render(score, True, pg.color.Color("WHITE"),True)  # returns text surface to show current score
        self.meta_screen.blit(text,(200,120)) # renders text surface to the display
    ...
    def chk_win(self):
        winner = 0
        for i in range(0, 3):
            if self.game_array[i] == self.game_array[i + 1] and self.game_array[i] == self.game_array[i + 2] and \
                    self.game_array[i] != 0 and (i == 0 or i == 3 or i == 6):
                print('pattern : i =', i)
                winner = self.game_array[i]
                break
            if self.game_array[i] == self.game_array[i + 3] and self.game_array[i] == self.game_array[i + 6] and \
                    self.game_array[i] != 0 and (i == 0 or i == 1 or i == 2):
                winner = self.game_array[i]
                print('pattern : i =', i)
                break
        ...
        if winner != 0:
            ...
            self.isActive = False
            self.drawScore()
            self.draw_not_Active_Screen(winner)

        elif self.moves == 9:
            print("TIE")
            self.isActive = False
            self.draw_not_Active_Screen(0)
    ...
    def fill_box(self,box_no):
        if self.game_array[box_no-1] == 0:
            ...
            self.invalid_move_sound.play()
            print("Invalid move")



    def draw_grid(self):
        pg.draw.line(self.screen, pg.color.Color("RED"), (0, 200), (600, 200), 5)
        pg.draw.line(self.screen, pg.color.Color("RED"), (200, 0), (200, 600), 5)
        pg.draw.line(self.screen, pg.color.Color("RED"), (0, 400), (600, 400), 5)
        pg.draw.line(self.screen, pg.color.Color("RED"), (400, 0), (400, 600), 5)


    def draw_not_Active_Screen(self,winner =0):
        self.screen.blit(self.game_state_screens[winner],(0,0))
